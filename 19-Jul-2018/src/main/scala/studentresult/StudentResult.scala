...
/*
case class StudentResult(Id:Int,Name:String,division:String,section:String) {

}
case class ScoreCard(Student_Id:Int,total_marks:Int,subject_1:Int,subject_2:Int,subject_3:Int){
  def totalMarks()= {
...
class StudentResult {
  ...
  def topThreeStudents(listOfStudents: List[Student], listOfScorecard: List[ScoreCard]): List[(Int, String, String, String)] = {
    require(listOfScoreCard.size >= 3, "List of Students and scorecard is not proper.")
    val scoreIdOfTopThreeStudents = listOfScoreCard.sortWith(_.totalMarks > _.totalMarks).slice(0, 3).map(_.id)
    scoreIdOfTopThreeStudents.map(studentId => listOfStudents.filter(_.id == studentId).head).map(stud => (stud.id, stud.name, stud.division, stud.section))
  }
  ...
  def displayFullMarksheet(studentId: Int, listOfStudents: List[Student], listOfScoreCard: List[ScoreCard]): String = {
    require(listOfScoreCard.nonEmpty, "No student exists.")
